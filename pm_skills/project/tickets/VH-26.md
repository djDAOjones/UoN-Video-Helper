...
- **Portrait: found and fixed 2026-08-28.** None of the five samples is
  portrait — all landscape, verified by `ffprobe`, none carrying a rotation
  flag — so this was never exercised. Reproduced with synthesised sources in
  both conventions a phone uses, then fixed and re-measured:

  | Source | No branding | With the closing card |
  | --- | --- | --- |
  | 1920x1080 coded + `rotation: 90` (what an iPhone writes) | worked | **failed**, now works |
  | 1080x1920 coded, no rotation flag | worked | worked |

  Cause and fix in `decision-log.md`. The composition is now specified as well
  as working: content fills the 1080x1920 frame edge to edge with rotation
  baked into the pixels, and the 16:9 closing card sits as a centred band on
  the brand background occupying 31% of the height — against the 31.6% the
  arithmetic gives for a 16:9 card in a 9:16 frame.
  Guarded by acceptance criterion 1, which covers both conventions; the
  unflagged one is included because it is the case that always worked, and a
  fixture that merely swaps width and height does not stand in for a phone
  file.

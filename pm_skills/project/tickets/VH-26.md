...
- **Portrait: found and fixed 2026-08-28.** None of the five samples is
  portrait — all landscape, verified by `ffprobe` — so it was never exercised.
  Reproduced with synthesised sources in both phone conventions, fixed, and
  guarded by acceptance criterion 1. Cause, fix and the measured composition
  are in `decision-log.md`.

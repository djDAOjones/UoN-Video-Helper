...
- **Portrait FAILS THE JOB.** Confirmed 2026-08-28. None of the five samples is
  portrait — all are landscape, verified by `ffprobe`, none carrying a rotation
  flag — so this was never exercised. Reproduced instead with two synthesised
  sources, one in each convention a phone uses:

  | Source | No branding | With the closing card |
  | --- | --- | --- |
  | 1920x1080 coded + `rotation: 90` (what an iPhone writes) | works | **fails** |
  | 1080x1920 coded, no rotation flag | works | works |

  The failure is `Video sample size must remain constant. Expected 1920x1080,
  got 1080x1920`, thrown by the encoder. **The closing card is on by default**,
  so this is every portrait phone upload, not an edge case.

  Not a rotation bug: without branding the same source produces a correct
  1080x1920 output with rotation baked to 0, so the content lane conforms
  properly. The two lanes disagree about the frame size — the branding lane
  appears to render at the asset's own 1920x1080 rather than at the portrait
  output shape. The exact ordering is for whoever fixes it; what is established
  is the reproduction and that it needs no acquired material.
